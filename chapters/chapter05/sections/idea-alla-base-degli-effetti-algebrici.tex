...
In questo però pensare agli effetti come semplici eccezioni risulta essere limitante. Per capire il problema, si osservi la seguente computazione che ricorre all'effetto dello stato:
\begin{lstlisting}[language=koka]
...
Il type system di Koka è in grado di capire che l'uso della variabile è limitato allo scope della funzione e la mutabilità non è osservabile dall'esterno; per questo motivo il tipo della funzione appare essere puro e privo di side effect.\mustfix{Aggiungere un paragrafo sull'uso opportunistico della mutabilità parlando di Koka!
